...
type WeekStats = {
  ...
  pending_review: number; // submitted + late_submitted
...
export default function LecturerWeeklyReportsPage() {
  ...
  // Load available periods for this lecturer
  const loadPeriods = useCallback(async () => {
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return;

      // Get all periods where this lecturer has students
      ...
      const uniquePeriodIds = [...new Set(registrations.map(r => r.period_id))];
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Load students assigned to this lecturer for the selected period
      const { data: registrations } = await supabase
        .from("student_registrations")
        .select(`
          id,
      ...
      // Load all weekly reports for these students
      ...
      for (let weekNum = 1; weekNum <= 13; weekNum++) {
        ...
        // Calculate statistics even if no reports exist
        ...
        // Get date range from first report of this week (or create estimated dates)
        ...
        if (weekReports.length > 0) {
          ...
          // Create estimated dates (assuming week starts from today for demo)
  ...
  const handleViewWeekDetails = async (week: WeekStats) => {
    ...
    try {
      ...
      // Load students for this lecturer in the selected period
      const { data: registrations } = await supabase
        .from("student_registrations")
        .select(`
          id,
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex-1">
            <h1 className="text-3xl font-bold text-gray-900">Báo cáo tuần - Timeline</h1>
            <p className="text-gray-500 mt-1">Theo dõi báo cáo theo 13 tuần thực tập</p>
          </div>
      ...
      {isLoading ? (
        ...
        <Card className="border-2 border-yellow-200">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-600">
              Bạn chưa được phân công hướng dẫn sinh viên nào trong bất kỳ kỳ thực tập nào.
            </p>
          </CardContent>
        </Card>
      ) : !selectedPeriodId ? (
        ...
        <Card className="border-2 border-gray-200">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-600">
              Bạn chưa có sinh viên nào trong kỳ thực tập này.
            </p>
          </CardContent>
        </Card>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard
              ...
              icon={Users}
              description="Đang hướng dẫn thực tập"
              color="blue"
            />
            <StatCard
              title="Tổng tuần"
            ...
            <StatCard
              ...
              icon={FileText}
              description="Tổng số báo cáo đã nhận"
              color="green"
            />
            <StatCard
              title="Chờ duyệt"
              ...
              icon={Clock}
              description="Báo cáo cần review"
              color="yellow"
            />
          </div>
            ...
            <CardContent>
              {stats.totalStudents === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">Chưa có sinh viên được phân công</p>
                  <p className="text-sm text-gray-400 mt-2">
                    Sinh viên sẽ xuất hiện khi admin phân công hoặc sinh viên tự chọn
                  </p>
                </div>
              ) : weekStats.length === 0 ? (
                <div className="text-center py-12">
            ...
            <CardContent className="space-y-3">
              <div className="flex gap-2">
                ...
                <p className="text-sm text-gray-700">
                  Click vào <strong>Xem chi tiết</strong> để xem danh sách sinh viên đã/chưa nộp báo cáo tuần đó
                </p>
              </div>
              <div className="flex gap-2">
                <div className="w-1.5 bg-blue-500 rounded-full shrink-0"></div>
                <p className="text-sm text-gray-700">
                  Sinh viên cần nộp tối thiểu <strong>8/13 tuần</strong> để hoàn thành thực tập
                </p>
              </div>
              <div className="flex gap-2">
                <div className="w-1.5 bg-blue-500 rounded-full shrink-0"></div>
                <p className="text-sm text-gray-700">
                  Tỷ lệ hoàn thành: <span className="text-green-600 font-semibold">≥80% (Tốt)</span>, 
                  <span className="text-yellow-600 font-semibold ml-1">≥50% (Trung bình)</span>, 
                  <span className="text-red-600 font-semibold ml-1">&lt;50% (Cần cải thiện)</span>
                </p>
              </div>
            </CardContent>
          </Card>
                        ...
                        {student.status !== "not_submitted" && (
                          <Button
                            ...
                            onClick={() => {
                              ...
                              router.push(`/dashboard/lecturer/weekly-reports/${student.registration_id}`);
                            }}
                          >
                            <Eye className="w-4 h-4 mr-1" />
                            Chi tiết

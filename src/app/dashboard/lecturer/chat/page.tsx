  ...
  if (conversations.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center p-6">
        ...
        <h3 className="text-lg font-semibold text-gray-700 mb-2">Chưa có sinh viên</h3>
        <p className="text-gray-500">
          Bạn chưa được phân công hướng dẫn sinh viên nào hoặc chưa có cuộc trò chuyện nào.
        </p>
      </div>
    );
  }
    ...
    <div className="flex h-[calc(100vh-4rem)] gap-4">
      ...
      <Card className="w-80 flex flex-col">
        <CardHeader className="border-b">
          <CardTitle className="text-lg">Sinh viên ({conversations.length})</CardTitle>
        </CardHeader>
          ...
          {conversations.map((conv) => (
            ...
            >
              <div className="flex items-start gap-3">
                <Avatar className="w-10 h-10 bg-green-100 text-green-700 flex items-center justify-center">
                  {conv.student_name.charAt(0).toUpperCase()}
                </Avatar>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <p className="font-semibold text-sm truncate">{conv.student_name}</p>
                    {conv.unread_lecturer_count > 0 && (
                      <Badge variant="destructive" className="ml-2 h-5 px-1.5 text-xs">
                        {conv.unread_lecturer_count}
                      </Badge>
                    )}
                  </div>
                  <p className="text-xs text-gray-500 mb-1">{conv.student_mssv}</p>
          ...
          <>
            <CardHeader className="border-b">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-lg">{selectedConversation.student_name}</CardTitle>
                  <p className="text-sm text-gray-500">{selectedConversation.student_mssv}</p>
                </div>
              </div>
            </CardHeader>
